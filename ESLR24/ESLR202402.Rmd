...
```{r, out.width='50%', fig.align='center', fig.cap='The agent–environment interaction in a Markov decision process',  echo=FALSE}
...
server <- function(input, output) {
  output$plot <- renderPlot({
  randomData <- rnorm(10000, mean = input$mean_adjust, sd = input$sd_adjust)
  hist(randomData, freq = TRUE,  breaks = 50,  xlim = c(0,10), ylim = c(0,1000),xlab = "Reward", ylab = "Frequency",  main = "Patch02 reward")
})
...
actions (not from correct instructions). Q-learning is based on the Temporal Differences and used to determine an
...
Toyokawa, W., Whalen, A., & Laland, K. N. (2019). Social learning
strategies regulate the wisdom and madness of interactive crowds. Nature
human behaviour, 3(2), 183-193.

Wilson, R. C., & Collins, A. G. (2019). Ten simple rules for the
computational modeling of behavioral data. Elife, 8, e49547.

Sutton, R. S. (2018). Reinforcement learning: An introduction. A
Bradford Book.

Newman, M. (2018). Networks. Oxford university press.

Barabási, A. L. (2013). Network science. Philosophical Transactions of
